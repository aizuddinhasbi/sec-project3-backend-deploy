...
export const updateTask = async (req, res) => {
  ...
  try {
    const updatedTask = await pool.query(
      "UPDATE tasks SET title = $1, description = $2, category = $3, priority = $4, completed = $5 WHERE id = $6 AND user_id = $7 RETURNING *",
      [title, description, category, priority, completed, id, req.userId]
    );
